...
"""
    * template engine, forked from https://github.com/lotabout/pymustache
    *
    *
    * @license  MIT http://opensource.org/licenses/MIT
    * @link     https://github.com/guangrei/lina
    """
...
# default filters
extensions = __import__("extensions")
filters = {}
...
# context now contains special variables: {'.': normal_context, '@':
# special_vars}
...
def delimiters_to_re(delimiters):
    ...
    if delimiters in re_delimiters:
        ...
        close_tag = ''.join(
            [c if c.isalnum() else '\\' + c for c in close_tag])
...
def compiled(template, delimiters=DEFAULT_DELIMITERS):
    ...
    while m is not None:
        ...
        if prefix == '=' and suffix == '=':
            ...
            if len(delimiters) != 2:
                raise SyntaxError(
                    'Invalid new delimiter definition: ' + m.group())
            re_tag = delimiters_to_re(delimiters)
            strip_space = True
        ...
        elif suffix != '' and suffix is not None:
            ...
            token = Section(
                sec_name,
                name,
                root=root) if prefix == '#' else Inverted(
                name,
                name,
                root=root)
        ...
        if strip_space:
            ...
            if pos:
                index = pos[1]
                if last_literal:
                    last_literal.value = last_literal.value.rstrip(
                        spaces_not_newline)
        ...
        if not refer_context:
            for name in names[1:]:
                try:
                    ...
                    name = parse_int(name) if isinstance(
                        value, (list, tuple)) else name
                    value = value[name]
                except BaseException:
                    # not found
                    value = xtn
